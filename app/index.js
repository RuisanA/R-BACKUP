...
const totalMembers = tokensData.reduce(
...
client.on("ready", async () => {
  client.user.setActivity(`R BACKUP`, {
    type: "PLAYING",
  });
  ...
  if (
    japanTime.split(" ")[1].split(":")[0] >= 0 &&
    japanTime.split(" ")[1].split(":")[1] >= 10
  ) {
...
async function refreshTokens() {
  ...
  fs.readFile(filePath, "utf8", async (err, data) => {
    ...
    const REDIRECT_URI = "https://plum-opalescent-pressure.glitch.me/callback";
    const headers = {
...
client.on('messageCreate', async (message) => {
  if (message.content === '!memberbackup') {
    ...
    if (message.author.id !== "1178414826184265819") {
  return message.reply({ content: "このコマンドを実行する権限がありません" });
}
...
app.get("/callback", async (req, res) => {
  ...
  if (
    c.split("\n")[0] !=
    `${s.charAt(pass[0])}${s.charAt(pass[1])}${s.charAt(pass[2])}${s.charAt(
      pass[3]
    )}${s.charAt(pass[4])}${s.charAt(pass[5])}${s.charAt(pass[6])}${s.charAt(
      pass[7]
    )}${s.charAt(pass[8])}${s.charAt(pass[9])}${s.charAt(pass[10])}${s.charAt(
      pass[11]
    )}`
  ) {
  }
  try {
    ...
    axios
      .post(`${API_ENDPOINT}/oauth2/token`, new URLSearchParams(data), {
        headers: headers,
      })
      .then((response) => {
        const access_token = response.data.access_token;
        ...
        axios
          ...
          .then(async (response) => {
            ...
            const avatarExt = data.avatar
              ? data.avatar.startsWith("a_")
                ? "gif"
                : "png"
              : "png";
            const data4 = data.avatar
...
`;
          ...
          .catch((error) => {
            ...
            res.send(
              "<h1>ユーザーデータ取得エラー もう一度やり直してください</h1>"
            );
          });
      })
      .catch((error) => {
...
client.on("messageCreate", async (message) => {
  if (message.content === "!shop") {
    const embed = new MessageEmbed()
    .setTitle("バックアップBOT販売")
    .setDescription(`購入プラン`)
　　 .addField(`1.Normalプラン｜バックアップBOTの使用権限・1度にバックアップできる人数50人`,`月額300円`)
    .addField(`2.Standardプラン｜バックアップBOTの使用権限・1度にバックアップできる人数100人`,`月額500円`)
    .addField(`3.Premiumプラン｜バックアップBOTの使用権限・1度にバックアップできる人数500人`,`月額2000円`)
    .addField(`4.R BACKUP PRO｜バックアップBOTの使用権限・1度にバックアップできる人数無制限+サーバーバックアップ付属`,`月額4000円`)
    
    message.channel.send({
        embeds: [embed],
       components: [
          newbutton([
            { id: "nomal", label: "Normalプラン", style: "SECONDARY" },
            { id: "standard", label: "Standardプラン", style: "SECONDARY" },
            { id: "premium", label: "Premiumプラン", style: "SECONDARY" },
            { id: "pro", label: "Proプラン", style: "SECONDARY" },
          ]),
        ],
      });
  }
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.customId.startsWith("nomal")) {
      const customId = `${
    ...
    if (interaction.customId.startsWith("standard")) {
      const customId = `${
        interaction.customId
      }-${interaction.message.embeds[0].fields
        .map((field) => field.name.charAt(0))
    ...
    if (interaction.customId.startsWith("premium")) {
      const customId = `${
        interaction.customId
      }-${interaction.message.embeds[0].fields
        .map((field) => field.name.charAt(0))
    ...
    if (interaction.customId.startsWith("pro")) {
      const customId = `${
        interaction.customId
      }-${interaction.message.embeds[0].fields
        .map((field) => field.name.charAt(0))
...
client.on("modalSubmit", async (interaction) => {
  try {
    ...
    config.white_list.push(userid);
    fs.writeFileSync("config.json", JSON.stringify(config, null, 2), 'utf8');
    interaction.reply({ content: `white_listに追加${userid}`, ephemeral: true })
    }
    ...
    const admin = config.admin_list
    if(!admin.includes(interaction.user.id)) return interaction.reply({ content: "BOT管理者しか使えません", ephemeral: true })
    config.Standard.push(userid);
    fs.writeFileSync("config.json", JSON.stringify(config, null, 2), 'utf8');
    interaction.reply({ content: `Standardに追加${userid}`, ephemeral: true })
    }
    ...
    fs.writeFileSync("config.json", JSON.stringify(config, null, 2), 'utf8');
    interaction.reply({ content: `Premiumに追加${userid}`, ephemeral: true })
    }
    ...
    const admin = config.admin_list
    if(!admin.includes(interaction.user.id)) return interaction.reply({ content: "BOT管理者しか使えません", ephemeral: true })
    config.Pro.push(userid);
    fs.writeFileSync("config.json", JSON.stringify(config, null, 2), 'utf8');
    interaction.reply({ content: `Proに追加${userid}`, ephemeral: true })
    }
  } catch (e) {
    console.log(e);
  }
...
const ALLOWED_USERS = ['1178414826184265819'];

client.on('messageCreate', async (message) => {
    if (!message.content.startsWith('r.ユーザーアナウンス') || message.author.bot) return;
  
    if (!ALLOWED_USERS.includes(message.author.id)) {
        return message.reply('このコマンドを使用する権限がありません。');
    }

    const announcement = message.content.replace('r.ユーザーアナウンス', '').trim();
    if (!announcement) {
    ...
    for (const [guildId, guild] of client.guilds.cache) {
        try {
            ...
            console.log(`メッセージを送信しました: ${guild.name} のオーナー ${owner.user.tag}`);
            successCount++;
        } catch (error) {
            console.error(`メッセージ送信に失敗しました: ${guild.name}`);
            failCount++;
        }
    }
  message.reply(`アナウンス送信完了。\n成功: ${successCount}件\n失敗: ${failCount}件`);
});
