...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let list = jsonData.map(obj => Object.keys(obj)[0]);
    
    if(config.white_list.includes(interaction.user.id)) {
      list = list.slice(0, 50);
    }
    
    if(config.Standard.includes(interaction.user.id)) {
      list = list.slice(0, 100);
    }
    
    if(config.Premium.includes(interaction.user.id)) {
      list = list.slice(0, 500);
    }
    
    if(config.Pro.includes(interaction.user.id)) {
    }
    ...
    for (let i = list.length - 1; i >= 0; i--) {
      ...
      if(i === 0){
        ...
        if(f === false) {
          config_.call_count.push({ [interaction.guild.id]: 1 });
        }
